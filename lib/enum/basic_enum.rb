...
class MyEnumeration
  ...
  def number_of_floats_or_fixnums
    collection.count{ |x| x if x.class == Fixnum || x.class == Float }
  end
  ...
  def square_numbers
    collection.map{ |num| num*num if num.class == Fixnum }
  end
  ...
  def find_first_awesome_person
    # collection.find { |object| object.has_value?(true) }
    collection.find { |object| object[:awesome] }
  end
  ...
  def element_frequency_count
    collection.each_with_object({}){ |i,h| h.store(i,collection.count(i)) }
  end
  ...
  def elements_ending_in_er
    collection.find_all { |item| item.index(?e,-2) && item.index(?r, -1) }
  end
  ...
  def sum_of_experiences
    collection.reduce(0) { |memo, object| memo += object[:years_experience] }
  end
